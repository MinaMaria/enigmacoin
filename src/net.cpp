    ...
    if (!fTestNet)
    {
        ...
        for (unsigned int seed_idx = 0; seed_idx < ARRAYLEN(strDNSSeed); seed_idx++) {
            if (HaveNameProxy()) {
				...
				CAddress addrnode = CAddress(CService("23.101.159.100", GetDefaultPort()));
				vAdd.push_back(addrnode);
				CAddress addrnode2 = CAddress(CService("185.90.163.251", GetDefaultPort()));
				vAdd.push_back(addrnode2);
